...
for i = 1 : trans
    ...
    for i = 1 : 4
        for j = 1 : M
            ...
            lsb = b(i);
            lsb(1) = 1.0 / 2.0 * (Gn * (x(3) - N0) - 1.0 / tauP) * x(1)+ kap * eDelay(delayIndex) * cos(theta);
            lsb(2) = alpha / 2.0 * (Gn * (x(3) - N0) - 1.0 / tauP) - kap * eDelay(delayIndex) / x(1) * sin(theta);
            lsb(3) = J - x(3) / tauS - Gn * (x(3) - N0) * x(1) * x(1);
        end
    end
    for i = 1 : M
        a(i) += h * (b(1)(i) + 2.0 * b(2)(i) + 2.0 * b(3)(i) + b(4)(i)) / 6.0;
    end
